...
public class Maquina {
	...
	public Pair<Integer, Integer> getBestChange(Instancia initialInstance, ArrayList<Pair<Integer, Integer> > tabuList) {
		Pair<Integer, Integer> toReturn = new Pair<Integer, Integer>(-1, -1);
		int minLatencia = Integer.MAX_VALUE;
		for(int i = 0; i < getTareasRealizadas().size() - 1; i++){
			for(int j = i + 1; j < getTareasRealizadas().size(); j++){
				...
				if(getLatencia(initialInstance) < minLatencia) {
					Pair<Integer, Integer> auxPair = new Pair<Integer, Integer>(getTareasRealizadas().get(i).getID(), getTareasRealizadas().get(j).getID());
					if(!tabuList.contains(auxPair) && !tabuList.contains(auxPair.inverse())) {
						toReturn = new Pair<Integer, Integer>(auxPair);
