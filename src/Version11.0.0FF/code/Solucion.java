...
public class Solucion {
	...
	public Pair<Integer, Integer> getBestExchangeFromSolution(Instancia instancia, ArrayList<Pair<Integer, Integer> > tabuList){
		...
		for (int i = 0; i < getMachineList().size() - 1; i++) {
			for (int j = i + 1; j < getMachineList().size(); j++) {
				auxBestExchange = getBestExchange(i, j, instancia, tabuList);
				aux = exchangeMachineItem(auxBestExchange.getLeft(), auxBestExchange.getRight());
				if(aux.getLatenciaTotal(instancia) < maxLatencia) {
					bestExchange = new Pair<Integer, Integer>(auxBestExchange.getLeft(), auxBestExchange.getRight());
	...
	public Pair<Integer, Integer> getBestExchange(Integer IDMaquina1, Integer IDMaquina2, Instancia instancia, ArrayList<Pair<Integer, Integer> > tabuList) {
		int ID1 = -1, ID2 = -1;
		Solucion aux = new Solucion(this);
		int maxLatencia = Integer.MAX_VALUE;
		for (int i = 0; i < getMachineList().get(IDMaquina1).getTareasRealizadas().size(); i++) {
			for (int j = 0; j < getMachineList().get(IDMaquina2).getTareasRealizadas().size(); j++) {
				...
				if(aux.getLatenciaTotal(instancia) < maxLatencia) {
					Pair<Integer, Integer> auxPair = new Pair<Integer, Integer>(getMachineList().get(IDMaquina1).getTareasRealizadas().get(i).getID(), getMachineList().get(IDMaquina2).getTareasRealizadas().get(j).getID());
					if(!tabuList.contains(auxPair) && !tabuList.contains(auxPair.inverse())) {
    ...
    public Pair<Integer, Integer> getBestIntramaquina(Instancia instancia, ArrayList<Pair<Integer, Integer> > tabuList) {
    	Pair<Integer, Integer> cambio = new Pair<Integer, Integer>(-1, -1);
    	int maxLatencia = Integer.MAX_VALUE;
    	for(int i = 0; i < getMachineList().size(); i++) {
    		Pair<Integer, Integer> auxCambio = getMachineList().get(i).getBestChange(instancia, tabuList);
    		if(auxCambio.getLeft() != -1 && auxCambio.getRight() != -1) {
	    		Solucion aux = new Solucion(this);
	    		aux.exchangeMachineItem(auxCambio.getLeft(), auxCambio.getRight());
	    		if(aux.getLatenciaTotal(instancia) < maxLatencia) {
	    			cambio = new Pair<Integer, Integer>(auxCambio);
	    			maxLatencia = aux.getLatenciaTotal(instancia);
	    		}
    		}
    	}
	...
	public void imprimirSolucion(String nombreDirectorio, Instancia initInstance, String algoritmo, double tiempoEjec, int nIteraciones){
		try{
			...
			ficheroResultado.println(parseFile(initInstance, algoritmo, tiempoEjec, nIteraciones));
	...
	public String toString() {
		...
		for (int i = 0; i < machineList.size(); i++) {
			aux += "\t\tMaquina " + (i+1) + ": ";
			for (int j = 0; j < machineList.get(i).getTareasRealizadas().size(); j++) {
				aux += machineList.get(i).getTareasRealizadas().get(j) + " ";
			}
			aux += "\n";
		}
